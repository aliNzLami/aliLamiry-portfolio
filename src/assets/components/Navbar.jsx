...
function Navbar({navsList}) {
    ...
    return (
        <>
            <nav ref={refNav} className='fixed z-50 flex flex-col justify-between w-full h-full uppercase bg-black text-white/80 py-5 md:py-10 ps-8 gap-y-18 md:w-1/2 md:left-1/2 borderNavbar'>
                <div className='flex flex-col text-5xl gap-y-3 md:text-5xl'>
                    {
                        navsList.map((item, index) => {
                            return(
                                <div key={item.title} ref={(element) => (refLinks.current[index] = element)}>
                                    <Link 
                                        to={item.title} 
                                        className='transition-all duration-300 cursor-pointer hover:text-[#d3a802]'
                                        offset={0}
                                        smooth
                                        duration={1300}
                                        onClick={toggleMenu}
